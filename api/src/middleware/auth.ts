...
// Demo bypass — only active outside production. Never merge a real secret here.
const IS_PROD = (process.env.NODE_ENV ?? 'development') === 'production';
export const DEMO_TOKEN = 'eso-demo-supervisor-2026';

const DEMO_PAYLOAD: TokenPayload = {
  sub: 'demo|supervisor',
...
export async function verifyToken(token: string): Promise<TokenPayload> {
  if (!IS_PROD && token === DEMO_TOKEN) return DEMO_PAYLOAD;
